...
class ThermostaticRadiatorValve extends TuyaSpecificClusterDevice {
  ...
  setWindowOpen(state) {
    ...
    this.writeBool(THERMOSTAT_DATA_POINTS.openWindow, state).catch(this.log);
  }
  ...
  applySettings(settings, keys) {
    ...
    if (keys.includes("openWindowTemperature")) {
      this.writeData32(
        THERMOSTAT_DATA_POINTS.openWindowTemperature,
        settings.openWindowTemperature * 10
      );
    }
    ...
    if (
      keys.includes("scheduleMonday") ||
      keys.includes("scheduleTuesday") ||
      keys.includes("scheduleWednesday") ||
      keys.includes("scheduleThursday") ||
      keys.includes("scheduleFriday") ||
      keys.includes("scheduleSaturday") ||
      keys.includes("scheduleSunday") ||
      keys.includes("workingDay")
    ) {
  ...
  updateSchedule(settings) {
    switch (settings.workingDay) {
      case "2": // separate
        ...
        var tuesdayBytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleTuesday,
          settings.scheduleTuesday
        );
        var wednesdayBytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleWednesday,
          settings.scheduleWednesday
        );
        var thursdayBytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleThursday,
          settings.scheduleThursday
        );
        var fridayBytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleFriday,
          settings.scheduleFriday
        );
      ...
      case "1": // Mon-Fri, Sat+Sun
        ...
        var saturdayBytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleSaturday,
          settings.scheduleSaturday
        );
        var sundaybytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleSunday,
      ...
      case "0": // Mon-Sun -> send monday schedule, used for all days
        var mondayBytes = marshalSchedule(
          settings.workingDay,
          THERMOSTAT_DATA_POINTS.scheduleMonday,
          settings.scheduleMonday
        );
